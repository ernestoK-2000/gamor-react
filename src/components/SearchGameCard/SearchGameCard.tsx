...
function SearchGameCard({ following, setFollowing }: Props) {
  ...
  function onFilterItemClick(event: React.PointerEvent<HTMLLIElement>) {
    closeFilterMenu();
    setCurrentGame(
      games.find((g) => g.name === (event.target as HTMLElement).innerText)
    );
  }
  ...
  let listFilter = games.map((g) => (
    <li onClick={onFilterItemClick} key={g.name}>
      {g.name}
    </li>
  ));
  return (
    <div id="sgc">
      <div id="sgc-header">
        ...
        <button
          ref={filterBtnRef}
          onBlur={onFilterMenuClose}
          onClick={onClickFilterBtn}
          id="filter"
          className="nude-btn"
        >
          <ul className="filter-settings">{listFilter}</ul>
